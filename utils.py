...
from imputers.missforest.missforest import MissForest
...
def read_dataset(dataset="iris", datapath=datapath):
    ...
    if dataset == "insurance":
        data = pd.read_csv(datapath + "Financial Datasets/insurance.csv")
        data = data.dropna().reset_index(drop=True)
        Xinds = list(range(0,6))
        yind = 6
    
    if dataset == "fraudulent_transactions":
        data = pd.read_csv(datapath + "Financial Datasets/Final Transactions.csv")
        data = data.dropna().reset_index(drop=True)

        #data manipulation
        fraud = data[data['TX_FRAUD'] == 1]
        non_fraud = data[data['TX_FRAUD'] == 0]
...
def iscategorical(x, threshold=0.12):
    ...
    if isinstance(x, pd.DataFrame):
        x = x.to_numpy()
    if x.dtype in ["object", "bool", "str"] and x.dtype != 'int64':
        return True
...
def get_num_cat_vars(df):
    ...
    for col in df.columns:
        if iscategorical(df[col], threshold=0.003):
            cat_vars.append(col)
        else:
            num_vars.append(col)
...
def one_hot_encode(X, X_miss, num_vars, cat_vars):
    ...
    #set the missing values in the encoded columns
...
def impute(
    ...
    imputers = {
        ...
        "KNNImputer": KNNImputer(n_neighbors=k_num, weights=weight),
        "MissForest": MissForest(initial_guess=initial_guess, max_iter=max_iter),
    }
    # initial_guess='median',
    # max_iter=1,
    # iteration=2,
    ...
    if method == "customMICE":
    ...
    elif method in ['KNNImputer']:
         #data scaling
        scaler = MinMaxScaler()
        scaled_df = pd.DataFrame(scaler.fit_transform(X_miss_le), columns=X_miss_le.columns)
        train_imp_scaled = imputer.fit_transform(scaled_df.iloc[train_idx])
        test_imp_scaled = imputer.transform(scaled_df.iloc[test_idx])
        # print(train_imp_scaled.shape)#(1003, 6)
        # print(test_imp_scaled.shape)
        train_imp = pd.DataFrame(
            scaler.inverse_transform(train_imp_scaled),
            columns=X_imp.columns,
        ).reindex(columns=X_miss_le.columns)
        test_imp = pd.DataFrame(
            scaler.inverse_transform(test_imp_scaled),
            columns=X_miss_le.columns,
        ).reindex(columns=X_imp.columns)
        X_imp.iloc[train_idx], X_imp.iloc[test_idx] = train_imp, test_imp
    elif method in ['MissForest']:
        # Miss Forest can naturally handle categories by splitting on them directly in the decision trees
        if cat_vars == []:
            train_imp = pd.DataFrame(
                imputer.fit_transform(X_miss.iloc[train_idx], categorical=None),
                columns=X_miss.columns,
            ).reindex(columns=X_miss.columns)
        else:
            train_imp = pd.DataFrame(
                imputer.fit_transform(X_miss.iloc[train_idx], categorical=cat_vars),
                columns=X_miss.columns,
            ).reindex(columns=X_miss.columns)

        test_imp = pd.DataFrame(
            imputer.transform(X_miss.iloc[test_idx]),
            columns=X_miss.columns,
        ).reindex(columns=X_miss.columns)
        ...
        raise ValueError(
            "method must be one of: zero, mean, median, mode, MICE, customMICE, KNNImputer."
        )

    # one hot decoding
    if encoding == "one-hot" and len(cat_vars) > 0 and method not in ['MissForest']:
...
def evaluate_imputation(X, X_imp, X_true, X_miss_idx, error_method="mix"):
    ...
    if error_method == "mse" or error_method == "rmse":
        ...
        cat_features = [x in cat_vars for x in X.columns]#mask for if cat is true
        error = mixerror(
            X_imp.to_numpy(),#convert from df to array
            X_true.to_numpy(),
            X_miss_idx.to_numpy(),
